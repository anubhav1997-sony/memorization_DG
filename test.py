...
@click.command()
@click.option('--savedir',                     help='Save directory',          metavar='PATH',    type=str, required=True,     default="/checkpoints/discriminator/cifar_uncond_test")
@click.option('--gendir',                      help='Fake sample directory',   metavar='PATH',    type=str, required=True,     default="/samples/cifar_uncond_vanilla_test")
@click.option('--datadir',                     help='Real sample directory',   metavar='PATH',    type=str, required=True,     default="/data/true_data_test.npz")
...
@click.option('--num_data',                    help='Num samples',             metavar='INT',     type=click.IntRange(min=1),  default=10000)
...
def main(**kwargs):
    ...
    discriminator = classifier_lib.load_discriminator(None, opts.device, opts.cond, eval=True)

    # discriminator = classifier_lib.get_discriminator(opts.pretrained_classifier_ckpt, "/checkpoints/discriminator/cifar_uncond3/discriminator_1000.pt",
    #                                                  0, opts.img_resolution, opts.device, enable_grad=False)
    ...
    discriminator.load_state_dict(torch.load("checkpoints/discriminator/t_1e5_standard_size/discriminator_50.pt"))
    discriminator.eval()
    ...
    with torch.no_grad():
        ...
        for data in train_loader:
            ...
            t = torch.tensor([1e-5 for i in range(inputs.shape[0])], device=inputs.device)
        ...
        label_predictions = torch.cat(label_predictions)
        print(label_predictions.shape)
        cors = ((label_predictions > 0.5).float() == all_labels).float().mean()
        label_predictions = label_predictions.detach().cpu().numpy()
        ...
        print(f" BCE loss: {loss_final}, correction rate: {cors}")

        

        plt.figure(figsize=(15,7))
        plt.hist(label_predictions[all_labels==0], bins=50, label='Fake')
        plt.hist(label_predictions[all_labels==1], bins=50, label='Real', alpha=0.7, color='r')
        plt.xlabel('Probability of being Positive Class', fontsize=25)
        plt.ylabel('Number of records in each bucket', fontsize=25)
        plt.legend(fontsize=15)
        plt.tick_params(axis='both', labelsize=25, pad=5)
        plt.savefig("conf_scores.png") 
        # ## Save
