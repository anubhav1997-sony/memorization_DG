...
def load_discriminator(ckpt_path, device, condition, eval=False, channel=512):
    discriminator_args = dict(
      image_size=8,
      classifier_use_fp16=False,
      classifier_width=128,#1024,#128,
      classifier_depth=2,#10,#100, #10
      classifier_attention_resolutions="32,16,8",
      classifier_use_scale_shift_norm=True,
      classifier_resblock_updown=True,
      classifier_pool="attention",
...
def get_grad_log_ratio_memorization(discriminator_L, discriminator_S, vpsde, unnormalized_input, std_wve_t, img_resolution, time_min, time_max, class_labels, log=False):
    mean_vp_tau, tau = vpsde.transform_unnormalized_wve_to_normalized_vp(std_wve_t) ## VP pretrained classifier
    if tau.min() > time_max or tau.min() < time_min or discriminator_L == None:
        if log:
          return torch.zeros_like(unnormalized_input), 10000000. * torch.ones(unnormalized_input.shape[0], device=unnormalized_input.device)
        return torch.zeros_like(unnormalized_input)
    else:
    ...
    with torch.enable_grad():
        ...
        if img_resolution == 64: # ADM trained UNet classifier for 64x64 with Cosine VPSDE
            tau = vpsde.compute_t_cos_from_t_lin(tau)
        tau = torch.ones(input.shape[0], device=tau.device) * tau
        log_ratio = get_log_ratio_memorization(discriminator_L, discriminator_S, x_, tau, class_labels)
        discriminator_guidance_score = torch.autograd.grad(outputs=log_ratio.sum(), inputs=x_, retain_graph=False)[0]
        # print(mean_vp_tau.shape)
        # print(std_wve_t.shape)
        # print(discriminator_guidance_score.shape)
...
def get_log_ratio_memorization(discriminator_L, discriminator_S, input, time, class_labels):
    if discriminator_L == None:
        return torch.zeros(input.shape[0], device=input.device)
    else:
        logits_L = discriminator_L(input, timesteps=time, condition=class_labels)
        logits_S = discriminator_S(input, timesteps=time, condition=class_labels)
        
        # print(logits.shape)
        prediction_L = torch.clip(logits_L, 1e-5, 1. - 1e-5)
        prediction_S = torch.clip(logits_S, 1e-5, 1. - 1e-5)
        
        log_ratio = torch.log(prediction_S / prediction_L)
        return log_ratio
