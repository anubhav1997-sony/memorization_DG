...
def edm_sampler_memorization_time_independent(
    ...
    return x_next, x_temp, x_0, x_time
...
def main(boosting, time_min, time_max, dg_weight_1st_order, dg_weight_2nd_order, cond, mem, discriminator_ckpt_l, discriminator_ckpt_s, pretrained_classifier_ckpt, discriminator_ckpt, save_type, batch_size, do_seed, seed, num_samples, network_pkl, outdir, class_idx, device, **sampler_kwargs):
    ...
    for i in tqdm.tqdm(range(num_batches)):
        ...
        if mem:
            images, images_tweedie, images_self, images_time = edm_sampler_memorization_time_independent(boosting, time_min, time_max, vpsde, dg_weight_1st_order, dg_weight_2nd_order, discriminator_l, discriminator_s, net, latents, class_labels, randn_like=torch.randn_like, **sampler_kwargs)   #  
        else:
        ...
        images_np_tweedie = (images_tweedie * 127.5 + 128).clip(0, 255).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
        images_np_self = (images_self * 127.5 + 128).clip(0, 255).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
        images_np_time = (images_time * 127.5 + 128).clip(0, 255).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()
        if save_type == "png":
            count = 0
            ...
            r = np.random.randint(1000000)
            with tf.io.gfile.GFile(os.path.join(outdir, f"samples_tweedie_{r}.npz"), "wb") as fout:
                io_buffer = io.BytesIO()
                if class_labels == None:
                    np.savez_compressed(io_buffer, samples=images_np_tweedie)
                else:
                    np.savez_compressed(io_buffer, samples=images_np_tweedie, label=class_labels.cpu().numpy())
                fout.write(io_buffer.getvalue())

            nrow = int(np.sqrt(images_np_tweedie.shape[0]))
            image_grid = make_grid(torch.tensor(images_np_tweedie).permute(0, 3, 1, 2) / 255., nrow, padding=2)
            with tf.io.gfile.GFile(os.path.join(outdir, f"sample_tweedie_{r}.png"), "wb") as fout:
                save_image(image_grid, fout)

            
            r = np.random.randint(1000000)
            with tf.io.gfile.GFile(os.path.join(outdir, f"samples_self_{r}.npz"), "wb") as fout:
                io_buffer = io.BytesIO()
                if class_labels == None:
                    np.savez_compressed(io_buffer, samples=images_np_self)
                else:
                    np.savez_compressed(io_buffer, samples=images_np_self, label=class_labels.cpu().numpy())
                fout.write(io_buffer.getvalue())

            nrow = int(np.sqrt(images_np_self.shape[0]))
            image_grid = make_grid(torch.tensor(images_np_self).permute(0, 3, 1, 2) / 255., nrow, padding=2)
            with tf.io.gfile.GFile(os.path.join(outdir, f"sample_self_{r}.png"), "wb") as fout:
                save_image(image_grid, fout)

            r = np.random.randint(1000000)
            with tf.io.gfile.GFile(os.path.join(outdir, f"samples_time_{r}.npz"), "wb") as fout:
                io_buffer = io.BytesIO()
                if class_labels == None:
                    np.savez_compressed(io_buffer, samples=images_np_time)
                else:
                    np.savez_compressed(io_buffer, samples=images_np_time, label=class_labels.cpu().numpy())
                fout.write(io_buffer.getvalue())

            nrow = int(np.sqrt(images_np_time.shape[0]))
            image_grid = make_grid(torch.tensor(images_np_time).permute(0, 3, 1, 2) / 255., nrow, padding=2)
            with tf.io.gfile.GFile(os.path.join(outdir, f"sample_time_{r}.png"), "wb") as fout:
                save_image(image_grid, fout)
